from __future__ import annotations

import cv2
import numpy as np

from .checkbox_extractor import CheckboxExtractor


class FieldExtractor:
    """Differential field extractor without local OCR engines."""

    def __init__(self) -> None:
        self.checkbox = CheckboxExtractor()

    def extract_fields(
        self,
        aligned_form: np.ndarray,
        interaction_mask: np.ndarray,
        field_definitions: list[dict],
    ) -> list[dict]:
        results: list[dict] = []
        for field_def in field_definitions:
            field_id = field_def["id"]
            field_type = field_def["type"]
            x, y, w, h = int(field_def["x"]), int(field_def["y"]), int(field_def["w"]), int(field_def["h"])
            img_h, img_w = aligned_form.shape[:2]
            x1, y1 = max(x, 0), max(y, 0)
            x2, y2 = min(x + w, img_w), min(y + h, img_h)
            if x2 <= x1 or y2 <= y1:
                results.append(self._empty(field_id, field_type, x, y, w, h))
                continue

            roi_form = aligned_form[y1:y2, x1:x2]
            roi_mask = interaction_mask[y1:y2, x1:x2]

            if field_type == "checkbox":
                result = self.checkbox.extract(roi_form, roi_mask)
            else:
                ink = cv2.bitwise_and(roi_form, roi_form, mask=roi_mask)
                density = float((roi_mask > 0).sum() / max(roi_mask.size, 1))
                result = {"value": "", "confidence": max(0.0, min(1.0, density)), "ink_preview": ink}

            results.append({"field_id": field_id, "field_type": field_type, "x": x, "y": y, "w": w, "h": h, **result})
        return results

    @staticmethod
    def _empty(
        fid: str, ftype: str, x: int, y: int, w: int, h: int
    ) -> dict:
        return {
            "field_id": fid,
            "field_type": ftype,
            "x": x,
            "y": y,
            "w": w,
            "h": h,
            "value": "" if ftype != "checkbox" else False,
            "confidence": 0.0,
        }
