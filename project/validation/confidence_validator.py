...
class ConfidenceValidator:
    ...
    def validate(self, extracted_fields: list[dict], field_definitions: list[dict]) -> list[dict]:
        ...
        for field in extracted_fields:
            ...
            above = conf >= self.threshold
            ok = sem["valid"]
            if above and ok:
                status = _ACCEPTED
                nr = False
            elif above and not ok:
                status = _SEM
                nr = True
            elif not above and ok:
                status = _LOW
                nr = True
            else:
                status = _REJ
